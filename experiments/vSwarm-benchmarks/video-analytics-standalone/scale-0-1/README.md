# Video Analytics Standalone Scale 0→1 Benchmark

Benchmark to evaluate scale-up performance from 0 to 1 instance under interfering load.
...
| `-b` | Base name of Video Analytics Standalone services to create | `video-analytics-standalone-python` |
...
| Flag | Description | Default |
|------|-------------|---------|
| `-f` | Name of the experiment | `video-analytics-standalone-benchmark` |
...
| `-c` | Number of concurrent Video Analytics Standalone Knative services to deploy | `5` |
| `-m` | Services manifest file base name | `kn-video-analytics-standalone-python` |
| `-p` | Services port | `80` |
| `-q` | Requests per second per service | `12` |
...
| `-b` | Base scale for the tested services | `0` |
...
### 1. Pull the images on Worker Node 1 and Worker Node 2
...
cd preempt-k8s/experiments/vSwarm-benchmarks/video-analytics-standalone/scale-0-1

# Deploy the database
kubectl apply -f ../video-analytics-standalone-database.yaml

# Wait for the database to be ready
# When the 'init-video-analytics-standalone-database' job
#   is completed, the database is ready
kubectl get pods -n default
```

### 3. Setup Invoker Pods and Service Manifests
...
Experiments will be stored in `/experiments/knative/vSwarm-benchmarks/video-analytics-standalone/kube-manager/<name-of-the-exp>`.
...
Experiments will be stored in `/experiments/knative/vSwarm-benchmarks/video-analytics-standalone/preempt-k8s/<name-of-the-exp>`.

### 5. Cleanup

Remove all invoker pods, services and database:
...
# Cleanup database
kubectl delete -f ../video-analytics-standalone-database.yaml
```
...
| Duration | 30 seconds |
| Iterations | 10 |
| Number of Interfering Resources | 15-30-45 |
| Base Scale per service | 0 |
...
kubectl cp  test-pod:/experiments/knative/vSwarm-benchmarks/video-analytics-standalone/kube-manager/<experiment-name> results/kube-manager/<experiment-name> -n default
cp -r /experiments/knative/vSwarm-benchmarks/video-analytics-standalone/preempt-k8s/<experiment-name> results/preempt-k8s/ -n default
```
...
python results.py results/kube-manager/scale-0-1_30-iter_kube-manager_5-15 5 kube-manager

# Process RTResource results
python results.py results/preempt-k8s/scale-0-1_30-iter_preempt-k8s_5-15 5 preempt-k8s
```
...
python compare-results.py \
  results/kube-manager/scale-0-1_30-iter_kube-manager_5-15 \
  results/preempt-k8s/scale-0-1_30-iter_preempt-k8s_5-15 \
  5
```
...
Processes a sensitivity analysis on varying interfering load (15, 30, 45 interfering resources).
...
python sensitivity-analysis.py \
  results/kube-manager/scale-0-1_30-iter_kube-manager_5-15 \
  results/kube-manager/scale-0-1_30-iter_kube-manager_5-30 \
  results/kube-manager/scale-0-1_30-iter_kube-manager_5-45 \
  results/preempt-k8s/scale-0-1_30-iter_preempt-k8s_5-15 \
  results/preempt-k8s/scale-0-1_30-iter_preempt-k8s_5-30 \
  results/preempt-k8s/scale-0-1_30-iter_preempt-k8s_5-45 \
  5
```
