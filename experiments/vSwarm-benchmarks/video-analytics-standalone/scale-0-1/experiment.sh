...
# Default values
EXPERIMENT_NAME="video-analytics-standalone-benchmark"
TEST_TYPE="Deployment"
INVOKER_POD_BASE_NAME="benchmark-pod"
INVOKER_PATH="/home"
NUMBER_OF_SERVICES="5"
SERVICE_MANIFEST_BASE_NAME="kn-video-analytics-standalone-python"
SERVICE_PORT="80"
RPS="12"
TIMEOUT="40"
DURATION="30"
...
while getopts "f:t:i:e:c:m:p:q:o:d:n:g:s:r:b:u:l:v:a:k:w:h" opt; do
    case $opt in
        ...
        h) 
            ...
            echo ""
            echo "Options:"
            echo "  -f <experiment-name>                Name of the experiment (default: video-analytics-standalone-benchmark)"
            ...
            echo "  -c <number-of-services>             Number of concurrent video analytics standalone Knative services to deploy (default: 5)"
            echo "  -m <service-manifest-base-name>     Services manifest file base name (default: kn-video-analytics-standalone-python)"
            echo "  -p <service-port>                   Services port (default: 80)"
            echo "  -q <requests-per-second>            Requests per second per service (default: 12)"
            ...
            echo "  -a <loki-ip-address>                IP address of the Loki service to query for control plane logs (default: 10.101.73.135)"
...
echo "================================================"
echo "vSwarm Video Analytics Standalone Benchmark - Starting"
echo "================================================"
...
if [ "$TEST_TYPE" == "Deployment" ]; then
    RESULTS_DIR="/experiments/knative/vSwarm-benchmarks/video-analytics-standalone/kube-manager/$EXPERIMENT_NAME"
elif [ "$TEST_TYPE" == "RTResource" ]; then
    RESULTS_DIR="/experiments/knative/vSwarm-benchmarks/video-analytics-standalone/preempt-k8s/$EXPERIMENT_NAME"
fi
...
================================================
vSwarm Video Analytics Standalone Benchmark - Test Configuration
================================================
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Waiting for video-analytics-standalone-python services base pods to be Running..."
while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | { grep "Running" || true; } | { grep "3/3" || true; } | wc -l) -ne $NUMBER_OF_SERVICES ]; do
    sleep 2
done
sleep 10
...
if [ "$BASE_SCALE" -eq 0 ]; then
    ...
    if [ "$FORCE_CLEANUP" == "true" ]; then
        TIMEOUT_SECONDS=90
        ELAPSED=0
        while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | { grep "Terminating" || true; } | wc -l) -ne $NUMBER_OF_SERVICES ]; do
        ...
        TERMINATING_PODS=$(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | { grep "Terminating" || true; } | awk '{print $1}')
        kubectl delete pods $TERMINATING_PODS --grace-period=0 --force &>/dev/null || true
    fi
    while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | wc -l) -gt 0 ]; do
        sleep 2
    done
...
for i in $(seq "$BASE_ITERATION" "$ITERATIONS"); do
    ...
    if [ "$FORCE_CLEANUP" == "true" ]; then
        TIMEOUT_SECONDS=90
        ELAPSED=0
        while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | { grep "Running" || true; } | wc -l) -ne $((BASE_SCALE*NUMBER_OF_SERVICES)) ]; do
            if [ $ELAPSED -ge $TIMEOUT_SECONDS ]; then
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Timeout waiting for pods to enter Terminating state"
                break
            fi
    ...
    for j in $(seq 1 "$NUMBER_OF_SERVICES"); do
        while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python-$j" || true; } | wc -l) -gt $BASE_SCALE ]; do
            sleep 2
        done
    done
...
================================================
vSwarm Video Analytics Standalone Benchmark Status - Iteration $i
================================================
Issued: $ISSUED
Completed: $COMPLETED
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Waiting for all video-analytics-standalone-python pods to terminate..."
if [ "$FORCE_CLEANUP" == "true" ]; then
    while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | wc -l) -gt 0 ]; do
        kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | awk '{print $1}' | xargs -r kubectl delete pod --grace-period=0 --force 2>/dev/null 2>&1 || true
        sleep 2
    done
else
    while [ $(kubectl get pods --no-headers | { grep "^video-analytics-standalone-python" || true; } | wc -l) -gt 0 ]; do
