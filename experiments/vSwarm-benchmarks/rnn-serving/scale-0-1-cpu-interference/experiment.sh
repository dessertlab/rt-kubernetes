...
if ! command -v stress-ng &>/dev/null; then
    echo "Error: stress-ng is not installed, please install it to run the cpu interfering load" >&2
    exit 1
fi
...
for i in $(seq "$BASE_ITERATION" "$ITERATIONS"); do
    ...
    # We run the stress-ng cpu interference in backgroung
    CPU_STRESS_PIDS=()
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] Starting cpu interference load..."
    PROCESSOR_COUNT=$(nproc)
    if [ "$PROCESSOR_COUNT" -gt 1 ]; then
        STRESS_PROCESSOR_COUNT=$((PROCESSOR_COUNT - 1))
    fi
    stress-ng --cpu "$STRESS_PROCESSOR_COUNT" &>/dev/null &
    CPU_STRESS_PIDS+=($!)
    ...
    # Clean up cpu interfering load
    for pid in "${CPU_STRESS_PIDS[@]}"; do
        kill -9 "$pid" > /dev/null 2>&1 || true
    done
