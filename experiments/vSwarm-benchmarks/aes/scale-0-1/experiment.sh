...
while getopts "f:t:i:e:c:m:p:q:o:d:n:g:s:r:b:u:l:v:a:k:w:h" opt; do
    case $opt in
        ...
        h) 
            ...
            echo ""
            echo "Options:"
            echo "  -f <experiment-name>                Name of the experiment (default: aes-benchmark)"
            ...
            echo "  -c <number-of-services>             Number of concurrent AES Knative services to deploy (default: 5)"
            echo "  -m <service-manifest-base-name>     Services manifest file base name (default: kn-aes-python)"
...
echo "================================================"
echo "vSwarm AES Benchmark - Starting"
echo "================================================"
...
if [ "$TEST_TYPE" == "Deployment" ]; then
    RESULTS_DIR="/experiments/knative/vSwarm-benchmarks/aes/kube-manager/$EXPERIMENT_NAME"
elif [ "$TEST_TYPE" == "RTResource" ]; then
    RESULTS_DIR="/experiments/knative/vSwarm-benchmarks/aes/preempt-k8s/$EXPERIMENT_NAME"
fi
...
================================================
vSwarm AES Benchmark - Test Configuration
================================================
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Waiting for aes-python services base pods to be Running..."
while [ $(kubectl get pods --no-headers | { grep "^aes-python" || true; } | { grep "Running" || true; } | { grep "3/3" || true; } | wc -l) -ne $NUMBER_OF_SERVICES ]; do
    sleep 2
done
sleep 10
...
if [ "$BASE_SCALE" -eq 0 ]; then
    ...
    if [ "$FORCE_CLEANUP" == "true" ]; then
        TIMEOUT_SECONDS=90
        ELAPSED=0
        while [ $(kubectl get pods --no-headers | { grep "^aes-python" || true; } | { grep "Terminating" || true; } | wc -l) -ne $NUMBER_OF_SERVICES ]; do
        ...
        TERMINATING_PODS=$(kubectl get pods --no-headers | { grep "^aes-python" || true; } | { grep "Terminating" || true; } | awk '{print $1}')
        kubectl delete pods $TERMINATING_PODS --grace-period=0 --force &>/dev/null || true
    fi
    while [ $(kubectl get pods --no-headers | { grep "^aes-python" || true; } | wc -l) -gt 0 ]; do
        sleep 2
    done
...
for i in $(seq "$BASE_ITERATION" "$ITERATIONS"); do
    ...
    if [ "$FORCE_CLEANUP" == "true" ]; then
        TIMEOUT_SECONDS=90
        ELAPSED=0
        while [ $(kubectl get pods --no-headers | { grep "^aes-python" || true; } | { grep "Running" || true; } | wc -l) -ne $((BASE_SCALE*NUMBER_OF_SERVICES)) ]; do
            if [ $ELAPSED -ge $TIMEOUT_SECONDS ]; then
                echo "[$(date '+%Y-%m-%d %H:%M:%S')] Timeout waiting for pods to enter Terminating state"
                break
            fi
    ...
    for j in $(seq 1 "$NUMBER_OF_SERVICES"); do
        while [ $(kubectl get pods --no-headers | { grep "^aes-python-$j" || true; } | wc -l) -gt $BASE_SCALE ]; do
            sleep 2
        done
    done
...
================================================
vSwarm AES Benchmark Status - Iteration $i
================================================
Issued: $ISSUED
Completed: $COMPLETED
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Waiting for all aes-python pods to terminate..."
if [ "$FORCE_CLEANUP" == "true" ]; then
    while [ $(kubectl get pods --no-headers | { grep "^aes-python" || true; } | wc -l) -gt 0 ]; do
        kubectl get pods --no-headers | { grep "^aes-python" || true; } | awk '{print $1}' | xargs -r kubectl delete pod --grace-period=0 --force 2>/dev/null 2>&1 || true
        sleep 2
    done
else
    while [ $(kubectl get pods --no-headers | { grep "^aes-python" || true; } | wc -l) -gt 0 ]; do
